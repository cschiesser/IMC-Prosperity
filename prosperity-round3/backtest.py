...
DATA_DIR = Path("data")
PRICE_FILES = [
    DATA_DIR / "prices_round_3_day_-1.csv",
    DATA_DIR / "prices_round_3_day_0.csv",
    DATA_DIR / "prices_round_3_day_1.csv",
]

TRADE_FILES = [
    DATA_DIR / "trades_round_3_day_-1.csv",
    DATA_DIR / "trades_round_3_day_0.csv",
    DATA_DIR / "trades_round_3_day_1.csv",
]
...
def main():
    ...
    for i in range(len(steps)):
        ...
        row = {"source_file": source_file, "day": day, "timestamp": timestamp}
        pnl_total = 0
        for product in PRODUCTS:
            mid = get_mid(order_depths[product])
            mark = mid if mid is not None else 0
            pnl = cashs[product] + positions[product] * mark
            pnl_total += pnl
            key = product.lower()
            row[f"{key}_position"] = positions[product]
            row[f"{key}_cash"] = cashs[product]
            row[f"{key}_mid"] = mid
            row[f"{key}_pnl"] = pnl
        row["total_pnl"] = pnl_total
    ...
    print("=" * 70)
    print(f"BACKTEST ABGESCHLOSSEN ({' + '.join(PRODUCTS)})")
    print("=" * 70)
    print(f"Anzahl Steps: {len(results)}")
    print()
    for product in PRODUCTS:
        key = product.lower()
        final_mid = results.iloc[-1][f"{key}_mid"] if len(results) > 0 else 0
        final_mark = final_mid if pd.notna(final_mid) else 0
        final_pnl = cashs[product] + positions[product] * final_mark
        print(product)
        print(f"  Finale Position: {positions[product]}")
        print(f"  Final Cash:      {cashs[product]}")
    ...
    print(f"  Average Total PnL: {results['total_pnl'].mean() if len(results) > 0 else 0:.2f}")
    print()
